...
custom_summary <- function(x) { list( mean_value = mean(x), median_value = median(x), sd_value = sd(x), min_value = min(x), max_value = max(x) ) }

# Fonksiyonu mtcars Veri Setine Uygulama

for (column_X in names(mtcars)) { if (is.numeric(mtcars[[column_X]])) { cat("for", column_X, ":\n") print(custom_summary(mtcars[[column_X]])) cat("\n") } }
